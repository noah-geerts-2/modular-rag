...
from .multimodal_loader_chunker import MultiModalLoaderChunker
...
class TestMultiModalLoaderChunker(unittest.TestCase):
  ...
  def setUpClass(cls):
    cls.llm = MagicMock()
    cls.chunker = MultiModalLoaderChunker(llm=cls.llm)
  ...
  def test_generate_ai_summary_with_valid_content(self):
    ...
    self.llm.create_completion.return_value = "AI generated summary of the document."
    summary = self.chunker.generate_ai_summary(content)

    # Assert: Check that the llm object was called with the correct arguments
    self.llm.create_completion.assert_called_once()
    self.llm.create_completion.assert_called_with(
      unittest.mock.ANY,
      images_base64=[]
    )
    self.assertEqual(summary, "AI generated summary of the document.")
