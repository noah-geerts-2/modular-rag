...
class TestChatGPT(unittest.TestCase):
    ...
    def test_create_completion_with_empty_images(self):
        # Arrange: Mock OpenAI response for prompt-only call
        mock_response = MagicMock()
        mock_response.choices = [MagicMock(message=MagicMock(content="response text"))]
        self.mock_client.chat.completions.create.return_value = mock_response

        # Act: Call create_completion with a prompt and an empty images_base64 list
        result = self.chat_gpt.create_completion("hello world", images_base64=[])
    ...
    def test_create_completion_with_empty_system_message(self):
        # Arrange: Mock OpenAI response for prompt-only call
        mock_response = MagicMock()
        mock_response.choices = [MagicMock(message=MagicMock(content="response text"))]
        self.mock_client.chat.completions.create.return_value = mock_response

        # Act: Call create_completion with a prompt and an empty string for system message
        result = self.chat_gpt.create_completion("hello world", system_message="")
    ...
    def test_create_completion_with_whitespace_system_message(self):
        # Arrange: Mock OpenAI response for prompt-only call
        mock_response = MagicMock()
        mock_response.choices = [MagicMock(message=MagicMock(content="response text"))]
        self.mock_client.chat.completions.create.return_value = mock_response
